...
impl Site for ITMediaEnterprise {
    fn name(&self) -> String {
        return "ITMedia Enterprise".to_string();
    }
    ...
    async fn parse_article(&mut self, url: &str) -> Result<(Html, Text)> {
        if url.starts_with("https://www.itmedia.co.jp/enterprise") == false {
            return Err(Error::msg(format!("URL unmatch: {}", url)));
        }
        let url = Url::parse(url).unwrap();
        let cookies = self.login().await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_itmedia_enterprise() {
        let mut site = ITMediaEnterprise::default();
