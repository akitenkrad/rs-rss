...
use kernel::{
    models::{
        list::PaginatedList,
        web_article::{
            Status, WebArticle, WebArticleFilteredOptions, WebArticleListOptions, WebSite, WebSiteListOptions,
        },
    },
...
impl WebSiteRepository for WebSiteRepositoryImpl {
    ...
    async fn delete_web_site(&self, id: WebSiteId) -> AppResult<()> {
        sqlx::query!(r#"DELETE FROM web_site WHERE site_id = $1"#, Uuid::from(id))
            .execute(self.db.inner_ref())
            .await
            .map_err(|e| shared::errors::AppError::SqlxError(e))?;
        Ok(())
...
impl WebArticleRepository for WebArticleRepositoryImpl {
    ...
    async fn select_filtered_web_articles(
        ...
        if let Some(ref kw) = keyword {
            query.push_str(&format!(
                " AND (wa.title LIKE '%{}%' OR wa.description LIKE '%{}%' OR wa.summary LIKE '%{}%')",
                kw, kw, kw
            ));
        }
        if let Some(ref start) = start_date {
            query.push_str(&format!(" AND wa.timestamp >= '{}'", start.naive_utc().date()));
        }
        if let Some(ref end) = end_date {
            query.push_str(&format!(" AND wa.timestamp <= '{}'", end.naive_utc().date()));
        }
        if let Some(ref st) = status {
            query.push_str(&format!(" AND wa.status = '{}'", st));
        }

        query.push_str(" ORDER BY wa.timestamp DESC");

        let rows = sqlx::query_as::<_, WebArticleRecord>(&query)
            .fetch_all(self.db.inner_ref())
            .await
            .map_err(|e| shared::errors::AppError::SqlxError(e))?;
    ...
    async fn update_web_article_state(&self, article_id: WebArticleId, new_status: Status) -> AppResult<WebArticle> {
        sqlx::query!(
            ...
            Uuid::from(article_id)
        )
        .execute(self.db.inner_ref())
        .await
        .map_err(|e| shared::errors::AppError::SqlxError(e))?;

        self.select_web_article_by_id(article_id).await
    }

    async fn delete_web_article(&self, id: WebArticleId) -> AppResult<()> {
        sqlx::query!(r#"DELETE FROM web_article WHERE article_id = $1"#, Uuid::from(id))
            .execute(self.db.inner_ref())
            .await
            .map_err(|e| shared::errors::AppError::SqlxError(e))?;
        Ok(())
...
mod tests {
    ...
    async fn test_website_crud(pool: sqlx::PgPool) {
        ...
        let records = repo.select_web_site_by_id(web_site.site_id).await.unwrap();
        ...
        // Delete
        repo.delete_web_site(updated_records.items[0].site_id).await.unwrap();
    ...
    async fn test_web_article_crud(pool: sqlx::PgPool) {
        ...
        // Read
        let records = repo.select_web_article_by_id(web_article.article_id).await.unwrap();
        ...
        // Delete
        repo.delete_web_article(updated_records[0].article_id).await.unwrap();
        let records_after_delete = repo.select_all_web_articles().await.unwrap();
        assert_eq!(records_after_delete.len(), 0);

        // Clean up the website
        web_site_repo.delete_web_site(web_site.site_id).await.unwrap();
    }
}
