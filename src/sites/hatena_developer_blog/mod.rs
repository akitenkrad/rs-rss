...
pub struct HatenaDeveloperBlog {}

#[cfg(test)]
mod tests;

impl Site for HatenaDeveloperBlog {
    fn name(&self) -> String {
        return "Hatena Developer Blog".to_string();
    }
    async fn get_articles(&self) -> Vec<WebArticle> {
        let body = reqwest::get("https://developer.hatenastaff.com/rss")
            .await
            .unwrap()
            .text()
            .await
