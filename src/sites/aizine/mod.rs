...
impl Site for AIZine {
    fn name(&self) -> String {
        return "AIZINE".to_string();
    }
    ...
    async fn get_articles(&self) -> Result<Vec<WebArticle>, String> {
        let body = self
            .request(&"https://otafuku-lab.co/aizine/feed/".to_string())
            .await;
    ...
    async fn get_article_text(&self, url: &String) -> Result<(Html, Text), String> {
        ...
        let selector = scraper::Selector::parse("#main article div.entry-content p").unwrap();
        let text = document
            .select(&selector)
